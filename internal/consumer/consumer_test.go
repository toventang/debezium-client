...
var configFile = flag.String("c", "../../cmd/etc/config.yaml", "set the config file, defaults to ~/etc/config.yaml")
...
func TestConsume(t *testing.T) {
	var (
		c      config.Config
		_      = config.LoadFromFile(*configFile, &c)
	...
	consumer := NewConsumer(context.Background(), logger, &c)
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			err := consumer.consume(kafka.Message{
				Key:   []byte("_"),
				Value: []byte(tc.event),
			})
